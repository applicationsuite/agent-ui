...
import type { IInputFieldControl, IFormOption, ITemplateControl } from '../../../templates.models';
...
export const FormFieldEditor = ({
  ...
  onUpdate: (
    controlId: string,
    partial: Partial<ITemplateControl>,
  ) => void;
  ...
  const isDuplicateName = existingNames.some(
    (n) => n === field.name && existingNames.indexOf(n) !== existingNames.lastIndexOf(n),
  );
    ...
    <>
      <CollapsibleSection title="Field Properties" classes={classes}>
        <FieldGroup label="Name" className={classes.field}>
          ...
          />
          {isDuplicateName && (
            <span style={{ color: tokens.colorPaletteRedForeground1, fontSize: tokens.fontSizeBase100 }}>
        ...
        <FieldGroup label="Input Type" className={classes.field}>
          <Dropdown
            ...
            onOptionSelect={(_, d) =>
              update({ inputType: d.optionValue as IInputFieldControl['inputType'] })
            }
          >
      ...
      <CollapsibleSection title="Validation" classes={classes} defaultExpanded={false}>
