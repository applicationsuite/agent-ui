...
export const ActionBindingInput = ({
  ...
  const effectivePlaceholder =
    placeholder || 'Type \u007B to insert bindings';
  const showSectionHeaders = filteredSections.length > 1;

  // Build flat index for highlight tracking
  let flatIdx = 0;
  const dropdownContent = (showDropdown || showEmpty) ? createPortal(
    <FluentProvider theme={fluentTheme}>
      ...
      >
        ...
        {showEmpty && <div className={classes.hint}>No matching fields</div>}
      </div>
    </FluentProvider>,
    document.body,
  ) : null;
  ...
  const previewSegments = useMemo(() => {
    if (!value) return [];
    const segments: { type: 'text' | 'binding'; content: string; valid: boolean }[] = [];
    const regex = /\{(\w+(?:\.\w+)*)\}/g;
    let lastIndex = 0;
        ...
        <div className={classes.preview}>
          {previewSegments.map((seg, i) =>
            seg.type === 'text' ? (
              ...
              <span
                key={i}
                className={seg.valid ? classes.bindingChip : classes.bindingChipInvalid}
              >
                {seg.content}
