...
export const useTreeDragDrop = (sections, onMoveNode, indicatorClasses) => {
    const dragItemRef = useRef(null);
    const [draggingId, setDraggingId] = useState(null);
    const [dropTarget, setDropTarget] = useState(null);
    const getDropPosition = useCallback((e, targetType) => {
    ...
    const findSectionById = useCallback((secs, id) => {
        for (const s of secs) {
            if (s.id === id)
                return s;
            if (s.subsections) {
    ...
    const handleDragOver = useCallback((e, targetId, targetType) => {
        ...
        setDropTarget((prev) => prev?.id === targetId && prev?.position === position
            ? prev
    ...
    const handleDrop = useCallback((e, targetNodeType, targetId, parentId, indexInParent) => {
        e.preventDefault();
        setDropTarget(null);
    ...
    const getDropClass = useCallback((id) => {
...
const findParentSectionId = (controlId, sections) => {
 ...
 */
export const useTreeOpenState = (selectedElement, sections, _cardChildren) => {
    const [openItems, setOpenItems] = useState(() => new Set(['card']));
    ...
    useEffect(() => {
        ...
        setOpenItems((prev) => {
            ...
            if (selectedElement.type === 'control') {
                ...
                const openParentChain = (sectionId, secs) => {
                    for (const s of secs) {
                        if (s.id === sectionId)
                            return true;
    ...
    const handleOpenChange = useCallback((_e, data) => {
 ...
 */
export const useTreeSelection = (selectedElement, onSelectElement) => {
    const isSelected = useCallback((type, id) => selectedElement?.type === type && selectedElement?.id === id, [selectedElement]);
    ...
    useEffect(() => {
        ...
        const handler = (e) => {
            ...
            const selectEl = target.closest('[data-select-type]');
            if (selectEl) {
