 ...
 */
export const extractBindingPaths = (obj) => {
    const paths = new Set();
 ...
 */
export const toPrettyFieldLabel = (field) => {
    if (!field)
        return 'New Tile';
 ...
 */
export const getSummaryTileKey = (tileId, idx) => {
 ...
 */
export const validateTemplateJson = (obj) => {
...
function validateSection(obj, path) {
    if (obj == null || typeof obj !== 'object' || Array.isArray(obj)) {
    ...
    if (s.subsections != null) {
        ...
        for (let i = 0; i < s.subsections.length; i++) {
            const err = validateSection(s.subsections[i], `${path}.subsections[${i}]`);
            if (err)
                return err;
        }
    }
...
function validateControl(obj, path) {
    if (obj == null || typeof obj !== 'object' || Array.isArray(obj)) {
        return `${path}: must be an object.`;
    }
    const c = obj;
