...
import { extractCustomPrompt, extractTemplateEntries, } from '../../../../AIAssistant.utils';
export const useResolveTemplate = ({ messageId, payload, userMessageText, resolveTemplate, skip, cachedResolved, onResolved, }) => {
    const [resolved, setResolved] = useState(cachedResolved);
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        void resolveTemplateRef.current(templateInfo, userMessageText, payload, customPrompt, abortController.signal)
            .then((result) => {
            if (!disposed) {
                setResolved(result);
