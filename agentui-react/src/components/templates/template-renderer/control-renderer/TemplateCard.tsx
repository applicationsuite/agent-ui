...
import type { ICardControl, IControlProps, ISectionControl, IInputFieldControl, ITemplateControl, IButtonControl } from '../../templates.models';
...
export const TemplateCard: React.FC<ITemplateCardProps> = (props) => {
              ...
              defaultValue: {
                ...
                value:
                  ...
                  typeof resolved === 'boolean'
                    ? (resolved as IInputFieldControl['defaultValue'] extends { value?: infer V } ? V : never)
                    : String(resolved ?? ''),
              },
  ...
  const cardHeaderActions = headerButtons.length > 0
    ? <>{renderButtons(headerButtons)}</>
    : undefined;

  const cardFooter = footerButtons.length > 0
    ? <>{renderButtons(footerButtons)}</>
    : undefined;
